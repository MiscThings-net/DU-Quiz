...
restart_quiz.onclick = ()=>{
    ...
    timeText.textContent = "남은 시간"; //change the text of timeText to Time Left
...
next_btn.onclick = ()=>{
    if(que_count < questions.length - 1){ //if question count is less than total question length
        ...
        startTimer(timeValue); //calling startTimer function
        startTimerLine(widthValue); //calling startTimerLine function
        timeText.textContent = "남은 시간"; //change the timeText to Time Left
        next_btn.classList.remove("show"); //hide the next button
    }else{
        clearInterval(counter); //clear counter
        clearInterval(counterLine); //clear counterLine
...
function showResult(){
    ...
    if (userScore > 9){ // if user scored more than 5
        //creating a new span tag and passing the user score number and total question number
        
        let scoreTag = `
            <div class="text-center">
                <img id="mainImg" src="img/00.png" alt="mainImage" class="img-fluid" width="100%" height="100%">
            </div>
            <div class="complete_text">
            <span><p>&nbsp&nbsp`+ questions.length +`</p>문제중  <p>`+ userScore +`</p>문제를 맞췄습니다.</span></div>`;
        ...
        let scoreTag = `
            <div class="text-center">
                <img id="mainImg" src="img/03.png" alt="mainImage" class="img-fluid" width="100%" height="100%">
            </div>
            <div class="complete_text">
            <span><p>&nbsp&nbsp`+ questions.length +`</p>문제중  <p>`+ userScore +`</p>문제를 맞췄습니다.</span></div>`;
        scoreText.innerHTML = scoreTag;
...
function startTimerLine(time){
    ...
    function timer(){
        ...
        if(time_line.style.width == "100%"){ //if time value is greater than 549
            clearInterval(counterLine); //clear counterLine
        }
    }
}
...
function queCounter(index){
    ...
    let totalQueCounTag = '<span><p>'+ index +'</p> of <p>'+ questions.length +'</p> 문제</span>';
...
function setShare() {
  ...
  const shareDes = `나의 학점은 `+ infoList[resultAlt] + ` 입니다.\n` + questions.length + `문제중 `+ userScore +`문제를 맞췄습니다.`;
  const shareImage = url + 'img/0' + resultAlt + '.png';
  const shareURL = url;
